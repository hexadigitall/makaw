...
class SongInfo {
  ...
  String get displayTitle => title.isNotEmpty ? title : filePath.split(Platform.pathSeparator).last.split('.').first;
  String get displayArtist => artist.isNotEmpty && artist != '<unknown>' ? artist : 'Unknown Artist';
  String get displayAlbum => album.isNotEmpty && album != '<unknown>' ? album : 'Unknown Album';
  String get fileName => filePath.split(Platform.pathSeparator).last;

  Map<String, dynamic> toJson() => {
    'id': id, 'title': title, 'artist': artist, 'album': album,
    'filePath': filePath, 'duration': duration, 'albumId': albumId, 'size': size,
  };

  static SongInfo fromJson(Map<String, dynamic> j) => SongInfo(
    id: j['id'] ?? 0, title: j['title'] ?? '', artist: j['artist'] ?? '',
    album: j['album'] ?? '', filePath: j['filePath'] ?? '',
    duration: j['duration'] ?? 0, albumId: j['albumId'] ?? -1, size: j['size'] ?? 0,
  );

  @override
  bool operator ==(Object other) =>
      identical(this, other) || other is SongInfo && id == other.id && filePath == other.filePath;
  @override
