...
class _MusicPlayerWidgetState extends State<MusicPlayerWidget> {
  ...
  void initState() {
    ...
    if (widget.service.allSongs.isEmpty && !widget.service.isScanning) {
      widget.service.scanAllSongs();
    }
  }
  ...
  Future<void> _fetchLyrics() async {
    final song = widget.service.currentSong;
    if (song == null) return;
  ...
  Widget _buildLyricsView() {
    ...
    if (_hasLrc) {
      final pos = widget.service.position;
      int activeIdx = 0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: widget.service.showNowPlaying && widget.service.currentSong != null
          ? _buildNowPlaying()
          : _isSearchPage
              ? _buildSearchPage()
              : Column(
                  children: [
                    ...
                    if (widget.service.hasQueue || widget.service.currentSong != null) _buildMiniPlayer(),
                    _buildFooter(),
                  ],
                ),
    );
          ...
          Row(
            children: [
              ...
              PopupMenuButton<String>(
                ...
                onSelected: (v) {
                  if (v == 'scan') widget.service.scanAllSongs();
  ...
  Widget _buildSearchPage() {
    ...
    final results = widget.service.allSongs.where((s) =>
  ...
  Widget _buildTabs() {
    ...
    return DefaultTabController(
      length: 5,
      initialIndex: ['Songs', 'Playlists', 'Folders', 'Albums', 'Artists'].indexOf(widget.service.selectedTab).clamp(0, 4),
      child: Column(
        children: [
          TabBar(
            onTap: (i) => widget.service.setSelectedTab(['Songs', 'Playlists', 'Folders', 'Albums', 'Artists'][i]),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(widget.service.scanError.isNotEmpty ? widget.service.scanError : 'No songs found',
              ...
              child: Row(
                ...
                children: [
                  FloatingActionButton(
                    heroTag: 'play_land',
                    onPressed: songs.isEmpty ? null : () => widget.service.playSong(0, fromList: songs),
                  ...
                  FloatingActionButton(
                    ...
                    onPressed: songs.isEmpty ? null : () => widget.service.playShuffled(songs),
              ...
              SizedBox(
                ...
                child: TextButton.icon(
                  onPressed: songs.isEmpty ? null : () {
                    widget.service.playShuffled(songs);
                  },
  ...
  Widget _sortBtn() {
    String label;
    switch (widget.service.sortMode) {
    ...
    return PopupMenuButton<String>(
      onSelected: (v) => widget.service.setSortMode(v),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
  ...
  Widget _songItem(SongInfo song, int index, List<SongInfo>? list) {
    final isCurrent = widget.service.currentSong?.id == song.id;
    final isFav = widget.service.isFavorite(song.id);
        ...
        child: ListTile(
          ...
          onTap: () {
            if (_selectMode) {
              ...
              widget.service.playSong(index, fromList: list);
            }
          },
        ),
      ),
  ...
  Widget _songMenu(SongInfo song, List<SongInfo>? list) {
    return PopupMenuButton<String>(
      ...
      onSelected: (v) {
        if (v == 'play') widget.service.playSongInfo(song);
        else if (v == 'play_next') widget.service.playNext(song);
        else if (v == 'add_queue') widget.service.addToQueue(song);
  ...
  Widget _playlistsTab(String search) {
    final lists = search.isEmpty
        ? widget.service.playlists
        : widget.service.playlists.where((p) => p.name.toLowerCase().contains(search)).toList();
        ...
        Expanded(
          child: ListView(children: [
            ListTile(
              ...
              subtitle: Text('${widget.service.favorites.length} songs', style: const TextStyle(color: Color(0xFF666680), fontSize: 12)),
              onTap: () {
                final f = widget.service.favorites;
                if (f.isNotEmpty) widget.service.playSong(0, fromList: f);
              },
            ),
            ...lists.map((pl) {
              final s = widget.service.getPlaylistSongs(pl.name);
              return ListTile(
                leading: Container(
                  width: 44, height: 44,
                  decoration: BoxDecoration(color: const Color(0xFF2A2A4E), borderRadius: BorderRadius.circular(8)),
                  ...
                  onSelected: (v) {
                    if (v == 'open') {
                      Navigator.push(context, MaterialPageRoute(builder: (_) => MusicPlaylistPage(
                        service: widget.service, playlistName: pl.name,
                      )));
                    } else if (v == 'play' && s.isNotEmpty) widget.service.playSong(0, fromList: s);
                    else if (v == 'delete') widget.service.deletePlaylist(pl.name);
                  },
                  itemBuilder: (_) => [
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              if (ctrl.text.isNotEmpty) { widget.service.addToPlaylist(ctrl.text, -1); Navigator.pop(ctx); }
            },
  ...
  Widget _groupTab(String search, String type) {
    ...
    IconData icon;
    switch (type) {
      case 'folders': map = widget.service.folders; icon = Icons.folder; break;
      case 'albums': map = widget.service.albums; icon = Icons.album; break;
      default: map = widget.service.artists; icon = Icons.person; break;
    }
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    TextButton.icon(
                      onPressed: () { widget.service.playSong(0, fromList: songs); Navigator.pop(context); },
  ...
  Widget _buildMiniPlayer() {
    final s = widget.service.currentSong;
    if (s == null) return const SizedBox.shrink();

    return GestureDetector(
      onTap: () => widget.service.setShowNowPlaying(true),
      child: Container(
        color: const Color(0xFF1A1A2E),
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Text(fmtDur(widget.service.position), style: const TextStyle(color: Color(0xFF666680), fontSize: 10)),
                      const SizedBox(width: 4),
                      Expanded(
                        child: LinearProgressIndicator(
                          value: widget.service.duration > Duration.zero
                              ? (widget.service.position.inMilliseconds / widget.service.duration.inMilliseconds).clamp(0.0, 1.0) : 0,
                          backgroundColor: const Color(0xFF2A2A4E),
                          valueColor: const AlwaysStoppedAnimation(Color(0xFF818CF8)),
                        ),
                      ),
                      const SizedBox(width: 4),
                      Text(fmtDur(widget.service.duration), style: const TextStyle(color: Color(0xFF666680), fontSize: 10)),
                    ],
                  ),
                ],
              ),
            ...
            IconButton(
              icon: Icon(widget.service.isPlaying ? Icons.pause_circle_filled : Icons.play_circle_filled,
                  color: const Color(0xFF818CF8), size: 32),
              onPressed: () => widget.service.togglePlayPause(),
            ),
          ],
        ),
      ),
      ...
      child: Row(
        children: [
          IconButton(
            ...
            onPressed: () => widget.service.setShowNowPlaying(false),
          ),
          const Spacer(),
          IconButton(
          ...
          IconButton(
            ...
            onPressed: () {
              widget.service.toggleFavorite(song.id);
      ...
      builder: (_) => DraggableScrollableSheet(
        ...
        builder: (_, scrollCtrl) => CustomScrollView(
          ...
          slivers: [
            ...
            if (widget.service.favorites.every((s) => s.id != songId))
              SliverToBoxAdapter(
                child: ListTile(
                  leading: const Icon(Icons.favorite, color: Color(0xFF818CF8)),
                  title: const Text('Favourites', style: TextStyle(color: Colors.white)),
                  onTap: () { widget.service.toggleFavorite(songId); Navigator.pop(context); _toast('Added to Favourites'); },
                ),
              ),
            SliverList(
              delegate: SliverChildBuilderDelegate(
                (_, i) {
                  ...
                  return ListTile(
                    ...
                    onTap: () { widget.service.addToPlaylist(pl.name, songId); Navigator.pop(context); _toast('Added to ${pl.name}'); },
                  );
                },
  ...
  Widget _npBody(SongInfo song, bool isFav) {
    ...
    Widget progressRow() => Row(children: [
      Text(fmtDur(widget.service.position), style: const TextStyle(color: Color(0xFF666680), fontSize: 12)),
      Expanded(
        child: SliderTheme(
          ...
          child: Slider(
            value: widget.service.duration > Duration.zero
                ? widget.service.position.inMilliseconds.toDouble().clamp(0, widget.service.duration.inMilliseconds.toDouble()) : 0,
            max: widget.service.duration > Duration.zero
                ? widget.service.duration.inMilliseconds.toDouble().clamp(1, double.infinity) : 1,
            onChanged: (v) => widget.service.seek(Duration(milliseconds: v.round())),
          ),
        ),
      ),
      Text(fmtDur(widget.service.duration), style: const TextStyle(color: Color(0xFF666680), fontSize: 12)),
    ]);
      ...
      child: Row(
        ...
        children: [
          IconButton(
            ...
            icon: Icon(Icons.shuffle, color: widget.service.isShuffled ? const Color(0xFF818CF8) : const Color(0xFF666680), size: 22),
            onPressed: () => widget.service.toggleShuffle(),
          ),
          const SizedBox(width: 8),
          IconButton(
            constraints: const BoxConstraints(minWidth: 40, minHeight: 40),
            ...
            onPressed: () => widget.service.previousSong(),
          ),
          const SizedBox(width: 8),
          Container(
            ...
            child: IconButton(
              ...
              padding: EdgeInsets.zero,
              icon: Icon(widget.service.isPlaying ? Icons.pause : Icons.play_arrow, color: Colors.white, size: 32),
              onPressed: () => widget.service.togglePlayPause(),
            ),
          ),
          const SizedBox(width: 8),
          IconButton(
            ...
            onPressed: () => widget.service.nextSong(),
          ),
          const SizedBox(width: 8),
          IconButton(
            constraints: const BoxConstraints(minWidth: 40, minHeight: 40),
            padding: EdgeInsets.zero,
            icon: Icon(
              widget.service.loopMode == LoopMode.off ? Icons.repeat :
              widget.service.loopMode == LoopMode.one ? Icons.repeat_one : Icons.repeat,
              color: widget.service.loopMode != LoopMode.off ? const Color(0xFF818CF8) : const Color(0xFF666680), size: 22,
            ),
            onPressed: () => widget.service.cycleLoopMode(),
          ),
        ],
      ),
    );
  ...
  void _queueSheet() {
    showModalBottomSheet(
      ...
      backgroundColor: const Color(0xFF0F0F1A),
      isScrollControlled: true,
      builder: (_) => _QueueSheetContent(service: widget.service),
    );
  }
  ...
  Widget _timerOpt(String label, int mins) {
    return ListTile(
      title: Text(label, style: TextStyle(color: widget.service.timerMinutes == mins ? const Color(0xFF818CF8) : Colors.white)),
      trailing: widget.service.timerMinutes == mins ? const Icon(Icons.check, color: Color(0xFF818CF8), size: 20) : null,
      onTap: () { widget.service.setTimer(mins); Navigator.pop(context); },
    );
  }

  void _speedSheet() {
    final speeds = [0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0];
    final cur = widget.service.player.speed;
    showModalBottomSheet(
      context: context,
      backgroundColor: const Color(0xFF1A1A2E),
      isScrollControlled: true,
            ...
            SliverList(
              delegate: SliverChildListDelegate(
                speeds.map((s) => ListTile(
                  ...
                  onTap: () { widget.service.player.setSpeed(s); Navigator.pop(context); },
                )).toList(),
              ),
            ),
            const SliverToBoxAdapter(child: SizedBox(height: 16)),
      ...
      builder: (_) => StatefulBuilder(
        builder: (ctx, setDlg) => DraggableScrollableSheet(
          ...
          builder: (_, sc) => CustomScrollView(controller: sc, slivers: [
            ...
            SliverToBoxAdapter(child: _settingsTile(Icons.refresh, 'Scan music folder', () { widget.service.scanAllSongs(); Navigator.pop(ctx); })),
            SliverToBoxAdapter(child: _settingsTile(Icons.sort, 'Sort: ${_sortLabel(widget.service.sortMode)}', () => _showSortPicker(ctx, setDlg))),
            SliverToBoxAdapter(child: _settingsTile(Icons.tune, 'Audio buffer: ${widget.service.bufferMs}ms', () => _showBufferPicker(ctx, setDlg))),
            SliverToBoxAdapter(child: _settingsTile(Icons.info, 'About Makaw Music', () => _showAbout(ctx))),
            SliverToBoxAdapter(child: _settingsTile(Icons.notifications, 'Notif: ${widget.service.notificationStatus}', () => _showAbout(ctx))),
      ...
      builder: (dCtx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: List.generate(modes.length, (i) =>
          RadioListTile<String>(
            ...
            value: modes[i], groupValue: widget.service.sortMode,
            activeColor: const Color(0xFF818CF8),
            onChanged: (v) { widget.service.setSortMode(v!); Navigator.pop(dCtx); setDlg(() {}); },
          ),
        )),
      ),
    );
      ...
      builder: (dCtx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: values.map((v) =>
          RadioListTile<int>(
            ...
            value: v, groupValue: widget.service.bufferMs,
            activeColor: const Color(0xFF818CF8),
            onChanged: (val) { widget.service.setBufferMs(val!); Navigator.pop(dCtx); setDlg(() {}); },
          ),
        ).toList()),
      ),
    );
