...
class _PdfMergerPageState extends State<PdfMergerPage> {
  ...
  Future<void> _mergeFiles() async {
    ...
    try {
      ...
      await Isolate.run(() => _mergePdfBytes(paths, outputPath));
  ...
  /// Performs the actual merge by concatenating raw PDF byte streams.
  /// This is a simplified approach — for production, use a proper PDF library.
  static Future<void> _mergePdfBytes(List<String> inputPaths, String outputPath) async {
    // Read all files
    ...
    if (allBytes.isEmpty) throw Exception('No valid PDF files found');

    // Simple concatenation approach (works for most PDFs)
    // For production: use syncfusion_flutter_pdf or similar
    final output = File(outputPath);
    final sink = output.openWrite();

    // Write first PDF header
    sink.add(allBytes.first);

    // For subsequent PDFs, we'd need proper PDF merging
    // This simplified version copies the first file as a base
    // In production, use a proper PDF merge library
    for (var i = 1; i < allBytes.length; i++) {
      // Append raw bytes (simplified — real implementation needs PDF structure)
      sink.add(allBytes[i]);
    }
            ...
            child: _files.isEmpty
                ? Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.picture_as_pdf, color: _kMuted, size: 48),
                        const SizedBox(height: 12),
                        Text('No PDF files added', style: TextStyle(color: _kMuted)),
                        const SizedBox(height: 8),
                        Text('Tap "Add Files" to select PDFs to merge',
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          leading: Container(
                            ...
                            decoration: BoxDecoration(color: _kAccent.withOpacity(0.15), borderRadius: BorderRadius.circular(6)),
          ...
          if (_files.isNotEmpty)
            Container(
              ...
              child: Column(
                children: [
                  if (_isMerging) ...[
                    LinearProgressIndicator(value: _mergeProgress > 0 ? _mergeProgress : null, backgroundColor: Colors.white12, valueColor: AlwaysStoppedAnimation(_kAccent)),
                    const SizedBox(height: 8),
                    const Text('Merging...', style: TextStyle(color: _kMuted, fontSize: 12)),
                    const SizedBox(height: 8),
                  ],
                  SizedBox(
