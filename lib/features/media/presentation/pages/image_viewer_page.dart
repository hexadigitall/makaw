...
import '../../data/services/image_viewer_service.dart';
...
class _ImageViewerWidgetState extends ConsumerState<ImageViewerWidget> {
  ...
  ImageViewerService get _service => ref.read(imageViewerServiceProvider) ?? ImageViewerService();

  @override
  void initState() {
    super.initState();
    _service.addListener(_onServiceChanged);
    if (_service.allImages.isEmpty && !_service.isScanning) {
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: _page == 'photos'
              ? RefreshIndicator(
                  onRefresh: () => _service.scanAllImages(),
          ...
          child: Center(
            child: Column(
              ...
              children: [
                ...
                Text(_service.isScanning ? 'Scanning...' : 'No images found',
  ...
  Widget _buildFoldersPage() {
    if (_service.folders.isEmpty) {
      return const Center(child: Text('No folders found', style: TextStyle(color: Color(0xFF666680))));
    }
    final folders = _service.folders.entries.toList()..sort((a, b) => b.value.length.compareTo(a.value.length));
    return ListView.builder(
  ...
  Widget _buildFolderContents() {
    ...
    final images = _service.folders[_selectedFolder] ?? [];
  ...
  Widget _buildViewer() {
    ...
    final isFav = _service.isFavorite(img.id);
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () { _service.toggleFavorite(img.id); setState(() {}); },
          ),
        ],
      ),
      body: Center(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              Navigator.pop(ctx);
              _service.addTrashEntry(img);
              _service.moveToTrash(img);
  ...
  Widget _buildTrashPage() {
    final items = _service.trashImages;
    if (items.isEmpty) {
          ...
          children: [
            ...
            Positioned(
              ...
              child: GestureDetector(
                onTap: () {
                  _service.restoreFromTrash(item);
                  _service.removeTrashEntry(item.id);
                  setState(() {});
                },
                child: Container(
